...
## Future addition: JSDoc / TypeDoc Portal

Common extension: publish API docs under `/docs/jsdoc` or `/docs/typedoc`.

### Recommended approach

1. Pick a generator:
   - **JSDoc** for JS-heavy projects
   - **TypeDoc** for TypeScript-first API docs
2. Output static HTML into `public/docs/jsdoc/` (or `public/docs/typedoc/`)
3. Add a portal card in `public/docs/index.html`
...
- If API docs are expensive to generate, only generate in CI/build.
- If you want local docs updates, add to `predev` (or run on demand).
